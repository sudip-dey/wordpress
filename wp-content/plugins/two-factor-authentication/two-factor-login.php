<?php
/*
Plugin Name: Two Factor Authentication (Premium) (Premium)
	...
	public function tfaVerifyCodeAndUser($user, $username, $password) {
		...
		if (is_wp_error($user)) {
			...
			if (is_wp_error($code_ok)) {
				...
				$ret =  new WP_Error('authentication_failed', '<strong>'.__('Error:', 'two-factor-authentication').'</strong> '.__('123The one-time password (TFA code) you entered was incorrect.', 'two-factor-authentication'));
			} elseif ($user) {
				$ret = $user;
			} else {
...
<!-- 					</p> -->
	...
	public function affwp_process_login_form() {
		if(!TWO_FACTOR_DISABLE) return true;
